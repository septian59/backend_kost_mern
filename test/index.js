...
describe("API ENDPOINT TESTING", () => {
  it("GET Landing Page", (done) => {
    chai
      ...
      .end((err, res) => {
        ...
        expect(res.body.hero).to.have.all.keys("user", "kost", "location");
  ...
  it("GET Detail Page", (done) => {
    chai
      .request(app)
      .get("/api/v1/member/detail-page/5e96cbe292b97300fc902226")
      .end((err, res) => {
        expect(err).to.be.null;
        expect(res).to.have.status(200);
        expect(res.body).to.be.an("Object");
  ...
  it("POST Booking Page", (done) => {
    const image = __dirname + "/bukti.jpg";
    const dataSample = {
      image,
    ...
    chai
      ...
      .attach("image", fs.readFileSync(dataSample.image), "bukti.jpg")
      .end((err, res) => {
        expect(err).to.be.null;
